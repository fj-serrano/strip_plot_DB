...
def select_archives():
    ...
    parser.add_argument("-db", "--databases", required = False, default = "/shared/bak/TFG/serrano/db", help = "Ruta de las copias locales de las bases de datos")
    parser.add_argument("-s", "--specie", required = False, default = "Hsa", help = "Especie utilizada en el analisis")
    args = parser.parse_args()
    file_path = args.input
    output_path = args.output
    db_path = args.databases
    specie = args.specie
    ...
    for root, dirs, files_in_dir in os.walk(file_path_clean):
        ...
        if os.path.basename(root).startswith("SRP"):
            study_loop = os.path.basename(root).split("_")[0]
            db = os.path.basename(root).split("_")[1]
            ...
            for subroot, subdirs, subfiles in os.walk(root):
                    ...
                    for file in subfiles:
                        ...
                        if file.endswith(".tsv") and file.startswith("counts_"):
                            file_path_full = os.path.join(subroot, file)
                            df = pd.read_csv(file_path_full, sep = "\t")
                            assing = file.split("_")[1].split(".")[0]
                            counts_estudios[f"{study_loop}_{db}_de_{assing}"] = len(df)
    return files, output_path, db_path, specie, counts_estudios

# FUNCIÓN PARA CREAR EL DATAFRAME CON LA INFORMACIÓN DE LOS ARCHIVOS SELECCIONADOS
def data_frame(files, percentil, db_path, specie, counts_estudios):
    ...
    db_size = count_mirnas_DB(db_path, specie)
    ...
    for file in files:
        ...
        total_mirnas = counts_estudios[f"{estudio}_{base_de_datos}_{assignment}"]
...
def count_mirnas_DB(files, especie):
...
def main():
    files, output_path, db_path, specie, counts_estudios = select_archives()
    if len(files) == 0:
    ...
    percentil = 80
    df = data_frame(files, percentil, db_path, specie, counts_estudios)
