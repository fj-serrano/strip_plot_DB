...
def select_archives():
    ...
    return files, output_path, db_path, specie

# FUNCIÓN PARA CREAR EL DATAFRAME CON LA INFORMACIÓN DE LOS ARCHIVOS SELECCIONADOS
def data_frame(files, percentil, db_path, specie):
...
def main():
    files, output_path, db_path, specie = select_archives()
    if len(files) == 0:
    ...
    percentil = 80
    df = data_frame(files, percentil, db_path, specie)
